...
import { CreditSummaryCard, CurriculumTree, CourseListTable } from "../curriculum";
...
export function CurriculumTab({
  ...
  const activeCurriculumItems = curriculum.filter(
    it => it.careerId === curCareerId && it.versionId === curVersionId
  );

  const handleAddCourseToCycle = (courseId: string, cycleNumber: number) => {
    const id = "curr_" + Date.now();
    const newItem: MpaCurriculumItem = {
      id,
  ...
  return (
    <PageTransition id="curriculum">
      <div className="space-y-6 text-left">
        ...
        {/* MAIN VISUAL EDITOR CANVAS - THE CYCLE GRID & CATALOG */}
        {curCareerId && curVersionId ? (
          <div className="space-y-6">
            <div className="p-3 bg-slate-100 rounded-lg text-[11px] font-bold text-slate-700 flex items-center gap-2 border border-slate-200">
              ...
              <span>Malla consultada: <strong>{selectedCareer?.name}</strong> | Versión del Plan: <strong>{selectedVersion?.name}</strong>. Puede reasignar ciclos o agregar asignaturas desde la tabla del catálogo.</span>
            </div>
            ...
            {/* 2. CATALOG SEARCH TABLE */}
            <CourseListTable
              courses={courses}
              curriculum={curriculum}
              curCareerId={curCareerId}
              curVersionId={curVersionId}
              onAddCourseToCycle={handleAddCourseToCycle}
            />
          </div>
        ) : (
          <div className="bg-white p-12 text-center rounded-xl border border-slate-200 shadow-xs text-slate-400 font-bold uppercase text-xs tracking-wider">
