...
import MpaDashboard from "../mpa/MpaDashboard";
import { Lock, ArrowLeft } from "lucide-react";
import Button from "../ui/Button";

interface MpaRouterProps {
  onLogout: () => void;
}

export default function MpaRouter(props: MpaRouterProps) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <span className="text-white text-xs font-bold uppercase tracking-widest">Validando Sesión de Planificación Académica (MPA)...</span>
      </div>
    );
  }
  ...
  return <MpaDashboard onLogout={props.onLogout} />;
}
